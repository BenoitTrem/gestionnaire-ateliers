...
<section class="min-h-screen bg-image relative">
    ...
    <div class="form-atelier-container">
        <div class="flex justify-between items-center">
            <h1 class="header-text_2">Modification de l'atelier " {{$atelier->nom}} "</h1>
            <a href="{{ route('ateliers.index') }}" class="button">
                Retourner à la liste
            </a>
        </div>
        <form method="POST" action="{{ route('ateliers.update', $atelier->id) }}">
            @csrf
            @method('PUT')

            <div class="form-group">
                <x-input-label for="nom" class="mt-12">Nom de l'atelier *</x-input-label>
                <input type="text" name="nom" id="nom" class="text-input" value="{{ old('nom', $atelier->nom) }}" maxlength="30" minlength="1" required>
                @error('nom')
                ...
                <select name="animateurs[]" id="animateur" multiple>
                    @foreach ($animateurs as $animateur)
                        <option value="{{ $animateur->id }}"
                            {{ (collect(old('animateurs', $atelier->animateurs->pluck('id')->toArray()))->contains($animateur->id)) ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <textarea name="description" id="description" class="text-input" style="resize: both; max-width: 100%; max-height: 200px;" maxlength="150" minlength="1" required>{{ old('description', $atelier->description) }}</textarea>
                @error('description')
                <p class="input-error">{{$message}}</p>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <select name="campus_id" id="campus_id" class="text-input">
                    <option value="" {{ old('campus_id', $atelier->campus_id) == '' ? 'selected' : '' }}>Aucun campus</option>
                    @foreach ($campuses as $campus)
                        <option value="{{ $campus->id }}" {{ old('campus_id', $atelier->campus_id) == $campus->id ? 'selected' : '' }}>
                            {{ $campus->nom }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select name="local_id" id="local_id" class="text-input">
                    <option value="" {{ old('local_id', $atelier->local_id) == '' ? 'selected' : '' }}>Aucun local</option>
                    @foreach ($locals as $local)
                        <option value="{{ $local->id }}" {{ old('local_id', $atelier->local_id) == $local->id ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <input type="url" name="url_inscription" id="url_inscription" class="text-input" value="{{ old('url_inscription', $atelier->url_inscription) }}">
            </div>
            ...
            <div class="form-group">
                ...
                <input type="date" name="date" id="date" class="text-input"
                       ...
                       value="{{ old('date', $atelier->date) }}" required>
                @error('date')
                <p class="input-error">{{$message}}</p>
                @enderror
            </div>
                ...
                <select type="number" name="heure_debut" id="heure_debut" class="text-input" required>
                    @for ($hour = 8; $hour <= 23; $hour++)
                        <option value="{{ sprintf('%02d:00', $hour) }}"
                            {{ old('heure_debut', $atelier->heure_debut) == sprintf('%02d:00', $hour) ? 'selected' : '' }}>
                            {{ sprintf('%02d:00', $hour) }}
                        </option>
                    @endfor
                </select>
            ...
            <div class="form-group">
                ...
                <input type="number" name="duree_minutes" id="duree_minutes" class="text-input" value="{{ old('duree_minutes', $atelier->duree_minutes) }}" max="300" min="30" required>
                @error('duree_minutes')
                <p class="input-error">{{$message}}</p>
                @enderror
            </div>
            ...
            <div class="button-container">
                <button type="submit" class="button mt-5">Modifier l'atelier</button>
            </div>
        </form>
    </div>
    <p class="p-28"></p>
